  ...
  const handleDelete = (id) => {
    axios({
      ...
      .catch((error) => {
        ...
        // alert창 대체
        dispatch(
          setWarningToast({ message: `레시피 삭제에 실패했습니다ㅠㅠ` })
        );
      });
  };
  ...
  return (
    <>
      ...
      <RecipeWrapper className={isLoading && "invisible"}>
        <Extra>
          <RecipeId>
            등록일 &nbsp;{dateConverter(recipe.createdAt)} &nbsp; 게시글 #
            {recipe.id}
          </RecipeId>
          <ButtonLike className="dark">
            <Link to={"/search"} state={{ prevPath: "/recipes" }}>
              목록으로 돌아가기
            </Link>
          </ButtonLike>
        </Extra>
        <Head>
          <HeadLeft>
            ...
            <HeadLeftBottom>
              <Info>
                <UserName
                  image={recipe.member.profileImagePath}
                  name={recipe.member.name}
                  className="large"
                />
