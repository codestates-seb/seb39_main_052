...
const MyFridge = () => {
  ...
  const getFridge = async () => {
    try {
      ...
      if (
        ...
        fridgeIngredients[0].name.length > 0
      ) {
  ...
  const handleDeleteAll = async (id) => {
    ...
    try {
      ...
      // alert창 대체
      dispatch(
        setWarningToast({ message: "냉장고를 비우는데 실패했어요ㅠㅠ" })
      );
    }
  };
  // 냉장고 취소 (새로고침)
  ...
  const handleSave = async () => {
    ...
    for (let i = 0; i < fridgeData.length; i++) {
      if (fridgeData[i].name.length <= 0) {
        // alert창 대체
        dispatch(
          setWarningToast({ message: `재료 이름은 빈칸이 될 수 없어요ㅠㅠ` })
        );
        break;
      }
      count++;
  ...
  return (
    <Container>
      ...
      <SubHead>
        <Help>
          ...
          <HelpMessage className="modal">
            <ul>
              <li>
                냉장고에 있는 재료를 기록하고 언제 어디서든 재료를 관리해보세요!
              </li>
              <li>
                재료 옆 아이콘( <StyledBulletPoint icon={faUtensils} /> ) 을
                클릭하여 해당 재료로 만들 수 있는 레시피를 바로 검색해보세요!
              </li>
              <li>
                우측의 + / - 버튼을 통해 재료를 추가하고 삭제할 수 있습니다
              </li>
              <li>
                재료의 색상은 유통기한에 따라 변합니다.
                <ul>
                  <li>
              ...
              <li>
                재료를 수정했다면 하단의 "냉장고 정리" 버튼을 눌러 재료를
                저장해주세요!
              </li>
            </ul>
          </HelpMessage>
      ...
      <ButtonWrap>
        ...
        <GeneralButton className="medium gray" onClick={handleCancel}>
          취소
        </GeneralButton>
        <GeneralButton
          className="medium"
          onClick={useConfirm(
            "정말 비우는건가요? 확인시 냉장고 상태를 되돌릴 수 없어요.",
            confirm,
            cancel
          )}
        >
        ...
        <GeneralButton className="medium" onClick={handleSave}>
          냉장고 정리
        </GeneralButton>
      </ButtonWrap>
    </Container>
