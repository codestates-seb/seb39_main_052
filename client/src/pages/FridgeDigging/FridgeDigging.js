...
const FridgeDigging = () => {
  ...
  return (
    <Container>
      <SearchWrapper>
        <Heading>제목으로 레시피 검색하기</Heading>
        <NameSearchBar />
        <Heading>재료로 레시피 검색하기</Heading>
        <TagSearchBar />
      </SearchWrapper>
      {/* 아무 검색어도 입력하지 않았을 때 */}
      ...
      {/* 일치하는 검색 결과가 없을 때 */}
      <Alert className={!isThereSearch || isThereResult ? "invisible" : null}>
        일치하는 결과가 없습니다. 검색 범위를 넓혀보는건 어떨까요?
      </Alert>
      {/* 일치하는 검색 결과가 있을 때 */}
      <Option className={!isThereSearch || !isThereResult ? "invisible" : null}>
        <ResultNum>
          총 <strong>{totalNum}</strong>개
        </ResultNum>
        <SortingTab sortMode={sortMode} setSortMode={setSortMode} />
      </Option>
      <RecipeWrapper
        className={!isThereSearch || !isThereResult ? "invisible" : null}
      >
